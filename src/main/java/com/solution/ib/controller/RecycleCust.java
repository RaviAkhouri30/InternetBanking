...
/**
 * Servlet implementation class RecycleCust
 */
@WebServlet("/RecycleCust")
public class RecycleCust extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		Logger LOGGER = Logger.getLogger(RecycleCust.class.getName());
		try {
		ArrayList<CustomerBean> dat = new ArrayList<CustomerBean>();
		PrintWriter out = response.getWriter();
		BankingDao dao = new BankingDao();
		dat = dao.getCustDetails(5);
		...
		while (idat.hasNext()) {
			...
			data = data + "<td><div class=\"view-button\" onclick=\"restore('"+bean.getCustID()+"')\">Restore</div></td>";
			data = data + "<td><div class=\"view-button\" onclick=\"del('"+bean.getCustID()+"')\">Delete</div></td>";
			data = data + "</tr>";
		}
		data = data +"</table>";
		out.print(data);
